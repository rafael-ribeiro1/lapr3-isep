...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    public Graph(Graph<V,E> other){
        this.numVert=other.numVert;
        this.numEdge=other.numEdge;
        this.isDirected=other.isDirected;
        this.vertices=new LinkedHashMap<>(other.vertices);
    }
    ...
    public Iterable<Edge<V,E>> incomingEdges(V vert){
        ...
        for (Map.Entry<V,Vertex<V,E>> entry: vertices.entrySet ()){
            Edge<V,E> edge=entry.getValue ().getEdge ( vert );
    ...
    public String toString() {
        StringBuilder s;
        if (numVert == 0) {
            s = new StringBuilder ( "\nGraph.AdjacencyMap.Graph not defined!!" );
        }
        else {
            s = new StringBuilder ( "Graph.AdjacencyMap.Graph: " + numVert + " vertices, " + numEdge + " edges\n" );
